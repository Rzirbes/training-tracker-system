...
export function InjuryListOptions({ athleteId, injuryId, tag }: Props) {
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent className='w-fit'>
        <DropdownMenuItem asChild>
          <Link href={buildingRouteWithId(RouteEnum.UPDATE_INJURY, athleteId, injuryId)}>
        ...
        <DropdownMenuItem asChild>
          <button
            ...
            onClick={() => {
              dialog.current?.open(
                <ConfirmDeleteDialog
                  title='Você tem certeza que deseja excluir essa lesão?'
                  route={'injuries/'.concat(injuryId)}
